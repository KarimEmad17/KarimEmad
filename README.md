# 🎮 Karim Hashad — Game Developer Portfolio

Unity developer specializing in gameplay programming, systems design, tools, multiplayer. This repo is a curated collection of my game projects, prototypes, and technical work.

📧 [kareememad612005@gmail.com] · 💼 [www.linkedin.com/in/karim-hashad-a592b31bb]

---

## 🕹️ Featured Projects

### [Kitchen Chaos]
![Project Banner](images/kitchen-chaos-banner.png)

One-line pitch: A fast-paced, time-pressured cooking simulation where you chop, fry, plate, and deliver dishes before the orders — and the clock — get the better of you.

Engine/Tools: Unity 2022 LTS, C#, State Machines, ScriptableObjects, UI/Audio Systems
Role: Solo Developer
Platform: PC
My contributions:

Designed and implemented the core cooking gameplay loop (chopping, frying, plating, trash/delivery stations)
Built a state-machine-driven kitchen counter/appliance system using ScriptableObjects for data-driven recipes
Implemented the dynamic order/recipe system with timed customer tickets and scoring
Built UI systems (order tickets, game timer, pause/game-over screens) and integrated sound effects/music
Handled player interaction and object pickup/drop logic across multiple station types

🔗 [Play on itch.io](#) · [Watch Trailer](#) · [Source Code](#)

---

### [Project Name 2]
![Project Banner](docs/images/project2-banner.png)

**One-line pitch:** ...

- **Engine/Tools:** ...
- **Role:** ...
- **Platform:** ...
- **My contributions:** ...

🔗 [Play](#) · [Trailer](#) · [Source](#)

---

### [Project Name 3]
**One-line pitch:** ...

- **Engine/Tools:** ...
- **Role:** ...
- **My contributions:** ...

🔗 [Play](#) · [Source](#)

---

## 🧪 Prototypes & Game Jams

| Project | Jam / Timeframe | Description | Link |
|---|---|---|---|
| [Jam Game 1] | Ludum Dare 56, 48hrs | Short description | [Play](#) |
| [Jam Game 2] | GMTK Jam 2025, 96hrs | Short description | [Play](#) |

---

## 🛠️ Technical Skills

**Languages:** C#, [C++, Python, etc.]
**Engines:** Unity (primary), [Unreal, Godot]
**Specialties:** Gameplay systems, AI/behavior trees, multiplayer/netcode, UI/UX implementation, tools & editor scripting, shaders/VFX
**Version Control:** Git, Git LFS, Perforce
**Other:** [Photon, Mirror, Addressables, DOTS/ECS, Mecanim, Cinemachine]


## 📄 Resume

[Download my resume](resume.pdf)

---

## 📜 License

Code samples in this repo are shared under the [MIT License](LICENSE) unless otherwise noted. Game assets, art, and audio are © [Your Name] and not licensed for reuse.
